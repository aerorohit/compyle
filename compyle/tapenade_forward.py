...
def get_diff_signature(f, active):
    ...
    for s in sig.parameters:
        ...
        if s not in active:
            ...
            pyb_py.append([sig_to_pyb_call(s, typ), sig_to_pyb_call(s+VAR_SUFFIX, typ)])
            pyb_c.append([sig_to_c_call(s, typ), sig_to_c_call(s+VAR_SUFFIX, typ)])
            pure_c.append(["{typ} {i}".format(typ=typ, i=s), "{typ} {i}".format(typ=typ, i=s + VAR_SUFFIX)])
...
def get_c_forward_diff(fn, active=None):
    source = get_source(fn)
    name = fn.__name__
    with open(name + '.c', 'w') as f:
        f.write(source)
    cwd = os.getcwd()
    command = "tapenade {name}.c -d -o {name}_forward_diff -tgtvarname {tgt_var} -tgtfuncname {tgt_func}".format(name=name, tgt_var=VAR_SUFFIX, tgt_func=FUNC_SUFFIX)
    print(command)
    # subprocess.Popen(command)
    os.system(command)

    diff_file = open(name + "_forward_diff_d.c", 'r' )
    diff_src = diff_file.read()
    diff_src = pyb11_setup_header + diff_src

    pyb_all, c_all, _, _ = get_diff_signature(fn, active)
    pyb_call = ", ".join(pyb_all)
    c_call = ", ".join(c_all)
    
    pyb_temp = Template(pyb11_wrap_template)
    pyb_bind = pyb_temp.render(name=name, func_suffix=FUNC_SUFFIX,pyb_call=pyb_call, c_call=c_call)
    
    all_source = diff_src + pyb_bind
    
    with open(name + "_fdiff.cpp", 'w') as f:
        f.write(all_source)
    
    module = cppimport.imp(name + '_fdiff')
    
    return getattr(module, name + FUNC_SUFFIX)
...
class Elementwise_Grad:
    ...
    def _generate(self):
        ...
        with open(self.name + '.c', 'w') as f:
            f.write(self.source)
        command = "tapenade {name}.c -d -o {name}_forward_diff".format(name=self.name)
        os.system(command)
        ...
        if self.backend == 'cython':
            raise NotImplementedError('Cython elementwise is not yet implemented')        
    ...
    def _c_gen(self):
        pyb_args, pyb_c_args, py_args, c_args = get_diff_signature(self.func, self.active)

        c_templt = Template(c_backend_template)
        c_code = c_templt.render(
            c_kernel_defn = self.grad_source,
            fn_name = '{fname}{suff}'.format(fname=self.name, suff=FUNC_SUFFIX),
